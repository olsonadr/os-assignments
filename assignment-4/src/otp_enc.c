...
int main(int argc, char ** argv)
{
    ...
    int * plain_buff_lens;
    int * key_buff_lens;
    int num_plain_buffs = 0;
    int num_key_buffs = 0;
    int result;

    // Get plaintext from files
    result = check_and_read_file(argv[1], &plain_buffers,
                                 &num_plain_buffs, &plain_buff_lens);
            ...
            && (plain_buff_lens[num_plain_buffs - 1] >= key_buff_lens[num_key_buffs - 1])))
    {
        ...
        e_error("Key file is shorter than plaintext!", 1);
    }
    ...
    send_message(socketFD, "otp_enc", strlen("otp_enc"));

    // wait for confirmation of receiving identity
    receive_message(socketFD, enc_buff, 10);
    int good_id = (strstr(enc_buff, "got id") != NULL);

    if (good_id)
    {
        // send each buffer of plaintext and key (always more or equal buffs of key)
        for (i = 0; i < num_plain_buffs; i++)
        {
            // send plaintext buffer
            send_message(socketFD, plain_buffers[i], plain_buff_lens[i]);

            // wait for confirmation of receiving plaintext
            receive_message(socketFD, enc_buff, 10);
            if (strstr(enc_buff, "got pt") == NULL) { error("error!"); break; }
